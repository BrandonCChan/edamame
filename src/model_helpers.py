...
def check_excel_file(excel_book):
    ...
    specification_df = pd.read_excel(excel_book, 'specification', header=None,
                                     index_col=0)  # TODO:// add specification checks
    ...
    check_multiple = transitions_df.groupby(['start_state', 'end_state']).size().reset_index().rename(
        columns={0: 'count'})
    if (check_multiple['count'] > 1).any():
        ...
        print()
        raise ValueError(
            'Multiple identical defined transitions found. Please check transitions sheet in input excel document')

    # Check for valid defined transition types
    valid_t_types = ['constant', 'beta', 'beta_r2p', 'gamma', 'residual',
    ...
    if len(invalid_t_types) > 0:
        ...
        print()
        raise ValueError(
            '>= 1 Invalid transition type specified. Please check transitions sheet in input excel document')
    ...
    for start_state in dirichlet_transitions.start_state.unique():
        ...
        if outbound_transitions.loc[outbound_transitions.type != 'dirichlet'].shape[0] > 0:
            raise ValueError('Not all outbound transitions for state:', start_state,
                             ' are of type dirichlet. Please check transitions sheet in input excel document')
    ...
    if np.setdiff1d(end_state_names, start_state_names).shape[0] > 0:
        raise ValueError('State missing entry:', np.setdiff1d(end_state_names, start_state_names).tolist(),
                         'please check transitions sheet in excel document')

    # Check for missing costs
    if np.setdiff1d(unique_states, costs_df['state'].tolist()).shape[0] > 0:
        raise ValueError('Missing cost value for state(s):', np.setdiff1d(unique_states, costs_df['state'].tolist()),
                         'please check cost sheet in excel document')

    # Check for missing utilities
    if np.setdiff1d(unique_states, utilities_df['state'].tolist()).shape[0] > 0:
        raise ValueError('Missing utility value for state(s):',
                         np.setdiff1d(unique_states, utilities_df['state'].tolist()),
                         'please check utilities sheet in excel document')
    ...
    if specification_df.loc['name_start_state'].values[0] not in unique_states:
        raise ValueError(
            'Start state must be a valid defined state in "transitions" sheet. Please check. Invalid entry:',
            specification_df.loc['name_start_state'].values[0])
...
def get_time_dependent_weibull(const, p, cycle, cycle_length, units):
    ...
    if units in scaling_mapping:
        ...
        raise ValueError('Unknown time unit provided for weibull time-dependent transition. Inputed value:', units,
                         ' Accepted values are [year, month, day]')
    ...
    if math.exp(-lmbda * (t1 ** p)) == 0 or (math.exp(-lmbda * (t2 ** p))) == 0:
        ...
        if tdtp > 1 or tdtp < 0:
            raise ValueError('Transition sampled is greater than 1 or less than 0. Sampled value:', round(tdtp, 4),
                             'at cycle:', cycle)
...
def get_time_dependent_gompertz(const, gamma, cycle, cycle_length, units):
    ...
    if units in scaling_mapping:
        ...
        raise ValueError('Unknown time unit provided for gompertz time-dependent transition. Inputed value:', units,
                         ' Accepted values are [year, month, day]')
    ...
    # adjusts to yearly x-axis. subtract 1 from t1 as model "starts at time 1" however first transition calculation is based off of t0 and t1
    tdtp = 1 - ((math.exp(-lmbda * gamma ** -1 * (math.exp(gamma * t2) - 1))) / (
        math.exp(-lmbda * gamma ** -1 * (math.exp(gamma * t1) - 1))))
...
def set_transition(transition_type, **kwargs):
    ...
    if transition_type == 'beta':
        ...
        if not all(parameter in kwargs for parameter in ('const', 'ancillary', 'cycle', 'cycle_length', 'units')):
            raise ValueError(
                'Incorrect inputs specified for time dependent. Need const, p, cycle, and cycle_length, and time units of curve.')
        ...
        if not all(parameter in kwargs for parameter in ('const', 'ancillary', 'cycle', 'cycle_length', 'units')):
            raise ValueError(
                'Incorrect inputs specified for time dependent. Need const, gamma, cycle, cycle_length, and time units of curve.')
        return get_time_dependent_gompertz(kwargs['const'], kwargs['ancillary'], kwargs['cycle'],
                                           kwargs['cycle_length'], kwargs['units'])
        ...
        if not all(parameter in kwargs for parameter in ('transition_matrix', 'update_index')):
            raise ValueError(
                'Incorrect inputs specified for residual calculation. Need transition_matrix and update_index')
